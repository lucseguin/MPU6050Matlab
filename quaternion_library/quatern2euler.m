...
%	27/09/2011    SOH Madgwick    Initial release
    ...
    R(1,1,:) = 2.*q(:,1).^2-1+2.*q(:,2).^2;       % 2.0*qw^2-1 + 2.0*qx^2
    R(2,1,:) = 2.*(q(:,2).*q(:,3)-q(:,1).*q(:,4));% 2.0(qx*qy - qw*qz)
    R(3,1,:) = 2.*(q(:,2).*q(:,4)+q(:,1).*q(:,3));% 2.0(qx*qz + qw*qy)
    R(3,2,:) = 2.*(q(:,3).*q(:,4)-q(:,1).*q(:,2));% 2.0(qy*qz - qw*qx)
    R(3,3,:) = 2.*q(:,1).^2-1+2.*q(:,4).^2;       % 2.0*qw^2-1 + 2.0*qz^2
